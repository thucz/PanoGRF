...
class App:
  ...
  def start(self, flags):
    ...
    try:
      ...
      self.args = args
      if args['use_lmdb']:
        self.full_width = args["mono_width"]
        self.full_height = args["mono_height"]
  ...
  def load_training_data(self):
    ...
    if args["dataset"] == "m3d":
        ...
        # m3d_dist=m3d_dist
      if args["use_lmdb"]:
        train_data = HabitatImageGeneratorFTMultiView_LMDB(
          args,
          split="train",
  ...
  def load_validation_data(self):
    ...
    if args["dataset"] == "m3d":
      if args["use_lmdb"]: #  == "eval_depth_test":
        ...
        val_data = HabitatImageGeneratorFTMultiView_LMDB(
          args,
          eval_mode,
          seq_len = args["seq_len"],
          reference_idx = args["reference_idx"],
  ...
  def run_depth_pose_carla(self, step, panos, depths, rots, trans, curr_idx):
    ...
    # if args["cost_volume"]:
    outputs = model.estimate_depth_using_cost_volume_v3_erp_multiview(panos_small, rots, trans, curr_idx, 
    ...
    if args["loss"] == "l1_cost_volume_erp":
      ...
      one_over_gt_depth = my_torch_helpers.safe_divide(1.0, depths_small[:, curr_idx]) #todo
      if args["std_uncert_tune"]:
        gt_dmap = depths_small[:, curr_idx].reshape(batch_size, 1, height, width).clone()
        ...
        if args["out_type"]=="disparity":
          loss1 = loss_lib.compute_l1_sphere_loss(
            outputs['raw_image_features'],
            one_over_gt_depth,
            mask=torch.gt(depths_small[:, curr_idx], 0.1))
    ...
    if step == 1 or \
      ...
      with torch.no_grad():
        ...
        depths_turbo = my_torch_helpers.depth_to_turbo_colormap(
          depths_small[:, curr_idx], min_depth=args["turbo_cmap_min"])
        normalized_depth_pred = depths_pred[:, 0]
        if args["normalize_depth"]:
          std, mean = torch.std_mean(depths_small[:, curr_idx],
                                      dim=(1, 2),
                                      keepdim=True)
        ...
        depth_abs_error_img = torch.abs(depths_small[:, curr_idx] -
        ...
        depth_abs_error_img_stacked = torch.cat(
          (panos_small[:, curr_idx], depths_turbo, depths_pred_turbo,
            depth_abs_error_img),
          dim=2)
        depth_mae = torch.mean(torch.abs(depths_small[:, curr_idx] -
                                          normalized_depth_pred),
                                dim=(1, 2, 3))
        depth_mse = torch.mean(torch.pow(
          depths_small[:, curr_idx] - normalized_depth_pred, 2.0),
          dim=(1, 2, 3))
  ...
  def log_training_to_tensorboard(self, step, run_outputs, curr_idx):
    ...
    if step == 1 or \
      ...
      with torch.no_grad():
        ...
        normalized_depth_pred = depths_pred[:, 0]
        if args["normalize_depth"]:
          std, mean = torch.std_mean(depths_small[:, curr_idx], dim=(1, 2),
                                     keepdim=True)
          normalized_depth_pred = loss_lib.normalize_depth(
            normalized_depth_pred,
            new_std=std,
  ...
  def run_training_loop(self):
    ...
    model = self.model
    pbar = tqdm(total=args["total_iter"], bar_format='{r_bar}')
    for epoch in range(args["epochs"]):
      for i, data in enumerate(train_dataloader):
        ...
        curr_idx = np.random.randint(0, args["reference_idx"])#args['seq_len']-1)
        ...
        #   continue
        self.log_training_to_tensorboard(step, run_outputs, curr_idx)
  ...
  def eval_on_training_data(self):
    ...
    with torch.no_grad():
      ...
      for i in range(num_iterations):
        ...
        depths_small = run_outputs["depths_small"][:, curr_idx]
    ...
    #todo
      ...
      if args["load_prepared_test_data"]:
        ...
        for i in range(args["test_sample_num"]):
          ...
          rgb = np.uint8(panos[:, curr_idx][0].data.cpu().numpy()*255)
