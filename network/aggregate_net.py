...
class DefaultAggregationNet(nn.Module):
    ...
    def __init__(self,cfg):
        ...
        # neuray_in_dim=32, in_feat_ch=32
        if "level" in self.cfg and self.cfg["level"] in [-1]: #img_feats level
            in_feat_ch = 16
        else:
            in_feat_ch = 32
        
        self.agg_impl = IBRNetWithNeuRay(dim, in_feat_ch=in_feat_ch, n_samples=self.cfg['sample_num'])
    ...
    def forward(self, prj_dict, que_dir):
        ...
        if 'wo_geometry' in self.cfg and self.cfg['wo_geometry']:
            # import ipdb;ipdb.set_trace()
            prob_embedding = torch.zeros_like(prob_embedding)

        dir_diff = get_dir_diff(prj_dir, que_dir)
        device = prj_dict['img_feats'].device

        if 'mask' not in prj_dict:
            valid_mask = torch.ones((qn*rn, dn, rfn, 1)).float().to(device)
        else:
            valid_mask = prj_dict['mask']
            
            # valid_mask = valid_mask.float() # rfn,qn,rn,dn
            # valid_mask = valid_mask.reshape(rfn, qn * rn, dn, -1).permute(1, 2, 0, 3)#(qn*rn, dn, rfn, 1)
        ...
        if "wo_appearance" in self.cfg and self.cfg['wo_appearance']:
            # import ipdb;ipdb.set_trace()
            prj_img_feats = torch.zeros_like(prj_img_feats)
