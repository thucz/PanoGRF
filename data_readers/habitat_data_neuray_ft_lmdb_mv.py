...
import random
class HabitatImageGeneratorFTMultiView_LMDB(torch.utils.data.Dataset):
  default_cfg={
      'seq_len': 3, 
  ...
  def __init__(self, args, split,
    ...
    if self.cfg["use_lmdb"]:
      self.full_width = self.cfg["mono_width"]
      self.full_height = self.cfg["mono_height"]
    ...
    # lmdb
    # for multiview
    env_path = args["save_dir"]+'/lmdb_render_'+split+"_"+str(args["mono_width"])+"x"+str(args["mono_height"])+"_seq_len_"+str(args["seq_len"])+ \
  ...
  def __getitem__(self, idx):#index
    ...
    data ={
      "rgb_panos": rgb_panos.copy(),
      "depth_panos": depth_panos.copy(),
      "rots": rots.copy(),
      "trans": trans.copy(),
      "rgb_cubes": rgb_cubes.copy(),
      "depth_cubes": depth_cubes.copy(),
      "rots_cubes": rots_cubes.copy(),
      "trans_cubes": trans_cubes.copy()
    }
    #augmentation
    ...
    # #   2: 0,
    # # }
    # test_views = self.cfg["test_views"] # We choose the center point as test view
    # ids_all = list(set(range(self.cfg["seq_len"])) - set(test_views)) # exclude test view
    # src_dict = {}
    # for idx in ids_all:
    #   src_dict[idx] = list(set(ids_all) - set([idx]))
    # if "render_cubes" in self.cfg and self.cfg["render_cubes"] and self.split == "train":  
    #   curr_idx = test_views[np.random.randint(len(test_views))] #
    #   render_ids = list(range(6*curr_idx, 6*(curr_idx+1)))      #
    ...
    # else:
    #   if self.split == "train":
    #     que_id = test_views[np.random.randint(len(test_views))] #
    #     ref_ids_all = ids_all
    #     np.random.shuffle(ref_ids_all) #
    #     src_ids = [src_dict[ref_id] for ref_id in ref_ids_all]
    #     print("que_id, ref_ids:", que_id, ref_ids_all)
    #   else:
    #     que_id = test_views[0] #test_views[np.random.randint(len(test_views))] #      
    #     ref_ids_all = ids_all
    #     src_ids = [src_dict[ref_id] for ref_id in ref_ids_all]
    #   # import ipdb;ipdb.set_trace()
