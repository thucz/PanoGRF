...
class HabitatImageGeneratorMultiView_LMDB(torch.utils.data.Dataset):
  default_cfg={
      'seq_len': 3, 
  ...
  def __getitem__(self, idx):#index
    ...
    ids_all = list(range(self.cfg["reference_idx"])) # we suppose the front viewpoints are input views
