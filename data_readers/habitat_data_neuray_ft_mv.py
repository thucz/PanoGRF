...
import cv2
import random
from data_readers.create_rgb_dataset_mv import RandomImageGeneratorMultiView
...
class HabitatImageGeneratorFTMultiView(torch.utils.data.Dataset):
  ...
  def __restart__(self):
    ...
    if self.vectorize:
      self.image_generator = RandomImageGeneratorMultiView(
        self.split,
  ...
  def __getitem__(self, item):#index
    ...
    data_orig = self.image_generator.get_sample(item, self.num_views, self.train)
    data = data_orig.copy()
    if self.split=="train":
      curr_idx = np.random.randint(0, self.num_views-1)# except for the last view(center viewpoint)
    else:
      curr_idx = 0
    data["curr_idx"] = torch.from_numpy(np.array([curr_idx]))
    if self.split=="train":
