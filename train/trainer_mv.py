    ...
    def _init_dataset(self):
        if self.cfg["train_dataset_type"] == "gen":
            ...
            if self.cfg["use_lmdb"]:
                from data_readers.habitat_data_neuray_lmdb_mv import HabitatImageGeneratorMultiView_LMDB
                train_set = HabitatImageGeneratorMultiView_LMDB(
                    args=self.cfg,
                    split=mode,
                    seq_len=self.cfg["seq_len"],
                    reference_idx=self.cfg["reference_idx"],
    ...
    def _init_val_dataset(self):
        if self.cfg["train_dataset_type"] == "gen":
            if self.cfg["use_lmdb"]:
                ...
                if self.cfg["debug"]:
                    mode="test"
                val_set = HabitatImageGeneratorMultiView_LMDB(
                    args=self.cfg,
                    split=mode,
                    seq_len=self.cfg["seq_len"],
                    reference_idx=self.cfg["reference_idx"],
